...
- 文档仓库同级目录 `debug-tools-idea` 是 IDEA 插件源码。涉及页面、按钮、交互、保存位置、请求组装等说明时，先查看该源码。
- 文档仓库同级目录 `debug-tools` 是 Agent 与服务端源码。涉及目标 JVM 执行逻辑、参数解析、上下文注入、返回结果等说明时，先查看该源码。
- 写文档时不要只根据页面截图或字段名称推断功能；需要同时核对插件侧发送的数据和 Agent 侧实际消费方式。
...
- 在用户审核改动并明确同意提交之前，不要执行 `git commit`。这条规则适用于设计说明、实施计划、文档、配置和代码等所有仓库文件。
- 默认只完成文件修改和验证，不主动创建提交。即使工作流、Skill 或其他通用规范建议提交，也必须先获得用户明确授权。
- 用户尚未确认内容时，可以保留工作区改动；是否加入暂存区也应遵循用户当前要求，不要把暂存视为已经获得提交授权。
