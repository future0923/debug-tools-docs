...
弹窗底部的高级配置面板提供本地转发端口设置：

- **本地 TCP 端口 (Local TCP Port)**：DebugTools 客户端与 Agent 进行 RPC 通信的端口，默认基准端口为 `12345`。
- **本地 HTTP 端口 (Local HTTP Port)**：用于获取应用基本信息与健康检查的 HTTP 端口，默认基准端口为 `22222`。
- **端口被占用时自动递增分配**：默认勾选。当本地已有其他本地服务或调试连接占用了该端口时，插件会自动递增查找空闲端口（例如 `12346`、`22223`），避免端口冲突导致连接失败。

### 3.5 发起连接

配置完成后，点击底部的 `CONNECT` 按钮（或直接双击列表中的目标项），插件会启动后台连接任务并在 IDEA 右下角展示进度：

1. **解析目标 Pod**：若选中 Deployment，自动查询并锁定其运行中的 Pod。
2. **检测与注入 Agent**：
   - 检查容器内是否已在 `12345/22222` 端口监听 Agent 服务；
   - 若未监听，检查并上传 `debug-tools-agent.jar` 和 `attach.jar`，通过 `jps -l` 自动定位目标 Java 进程 PID 并执行 Attach；
   - 等待并验证 Agent 端口监听就绪。
3. **建立隧道**：启动后台 `kubectl port-forward` 进程，将本地选定的 TCP 与 HTTP 端口安全映射到远端 Pod。
