...
`方法脚本` 页签选择 Method Around 脚本。脚本可以在目标方法调用前后处理参数、结果或上下文，适合临时补齐登录态、环境变量、ThreadLocal 等调用前置条件。

选择脚本后，当前连接会记录默认脚本名称。之后在同一连接中打开其他方法调用页，会优先带出这个默认脚本；如果单个方法缓存里保存过脚本，则以方法缓存为准。
