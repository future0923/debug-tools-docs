...
方法调用工作台外层由 DebugTools 工具窗口承载。左侧竖向工具栏用于在连接管理、方法调用、调用历史、SQL 历史等页面之间切换；切换到方法调用页后，顶部会显示已经打开的请求标签页。

![method_invoke_window_toolbar.png](/images/method/method_invoke_window_toolbar.png){v-zoom}

请求标签页右键菜单用于管理当前方法调用工作台中的标签页：

| 菜单 | 作用 |
| --- | --- |
| `关闭` | 关闭当前选中的方法请求标签页。 |
| `关闭其他标签页` | 保留当前标签页，关闭其他已经打开的方法请求标签页。 |
| `关闭所有标签页` | 关闭当前方法调用工作台中的全部请求标签页。 |
| `关闭左侧标签页` | 关闭当前标签页之前打开的请求标签页；左侧没有可关闭标签时不可用。 |
| `关闭右侧标签页` | 关闭当前标签页之后打开的请求标签页；右侧没有可关闭标签时不可用。 |
| `标签布局 -> 单行展示` | 所有请求标签页在一行内展示，标签较多时通过滚动或折叠菜单切换。 |
| `标签布局 -> 多行展示` | 请求标签页较多时换行展示，方便直接看到更多已打开方法。 |

标签布局也可以在 `Settings | DebugTools | 调用方法` 中设置默认值；右键菜单中的切换会立即作用于当前已经打开的方法调用工作台。
